...
export default (server) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join", (roomName) => {
      ...
      const roomInfo = {
        roomResult: results[roomName],
        roomScore: scores[roomName],
        roomCurRound: curRound[roomName],
      };
      io.to(roomName).emit("room_info", roomInfo);
    ...
    socket.on("select_card", (roomName, team, round, choice) => {
      ...
      io.to(roomName).emit("show_select", results[roomName]);
      io.to(roomName).emit("room_info", results[roomName]);
      console.log(results[roomName], scores[roomName], curRound[roomName]);
      const roomInfo = {
        roomResult: results[roomName],
        roomScore: scores[roomName],
        roomCurRound: curRound[roomName],
